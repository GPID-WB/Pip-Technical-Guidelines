...
This article documents the integration pattern for calling mAI Factory from **R/Shiny apps deployed on Posit Connect**. If you need to call mAI Factory from a **local Python script** instead, see the [Desktop Setup guide](mAI_factory_API_setup_prerequisities.qmd).
...
> The [Desktop Setup guide](mAI_factory_environment_setup.qmd) authenticates via the `itsai-platform` SDK's interactive browser login (`DesktopToken`). This chapter uses Posit Connect's OAuth integration instead, which is the right approach for headless Shiny apps that cannot open a browser.
...
> **Note:** This authentication path does **not** require the ITSAI on-boarding request described in the [Desktop Setup guide](mAI_factory_setup_access_request.qmd). Posit Connect handles identity through its own OAuth integration.
...
| Request ITSAI platform access (Desktop) | [On-Boarding Request](mAI_factory_setup_access_request.qmd) |
| Configure credentials and test a Python call | [Environment Setup](mAI_factory_environment_setup.qmd) |
