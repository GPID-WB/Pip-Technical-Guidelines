...
1. Create a new custom agent and select `User Data`.
2. Set the name to `gpid-planner`.
3. Open the file `gpid-planner.agent.md` from your local clone of `copilot-prompts` in another session:
   `copilot-prompts\.github\agents\gpid-planner.agent.md`
4. Copy the entire file content and paste it into the new agent editor in Positron you just created, then save.
5. Restart Positron.
