...
pwd
```
Make sure you’re in the right place. For example, `/Data/pip/files/data` is where PIP data is stored.
If you're not then, then type

```bash
cd /Data/pip/files/data
```

To the disk usage type this.
```bash
df -h | grep Data 
```

`df -h` lists all mounted filesystems and their disk usage in a human-readable format (e.g., GB, MB).
The output is piped (`|`) to `grep Data`, which filters and shows only the lines that contain the word "Data".
...
To see the total size of each folder (directory), use:
```bash
...
sudo docker ps -a
```
If the container is not running, inspect what is going on
```bash
sudo docker logs povertyscoreapi
```


If it is running, you can inspect one container (replace NAME_OR_ID)
```bash
...
## Deployment OK, BUT API is not working

### See the exact error (no filesystem poking)

```bash
# 1) What happened last time?
sudo docker logs --tail 200 povertyscoreapi
```
```bash
# 2) What happened when launched
sudo docker logs povertyscoreapi | head -n 20
```

### Reproduce interactively to watch the error live

To run R code on the VM, start a new container interactively with `/bin/bash` as the entrypoint.  
This gives you a shell prompt **inside** the container, where you can execute commands directly.
...
sudo docker run --rm -it \
  ...
  itsesippscoreregistrydev.azurecr.io/povertyscoreapi:latest \
  /bin/bash
...
```bash
Rscript -e "1+1"
Rscript -e "print(getRversion())"
```
When you’re done, type exit (or hit Ctrl+D) → that container stops and is removed (because of --rm).


```bash
# Inside the container:
which Rscript
ls -l /app/main.R /Data
Rscript /app/main.R   # <-- watch the stack trace
```
That launches the API in the foreground, attached to your terminal. When you hit Ctrl+C or close the shell, the process (and API) dies.

### Delete old images
You should always have only ONE image running in the VM. If you have multiple images, you should delete the old ones.

check how many images you have on the VM with 
```bash
sudo docker images
```

Delete the old ones with
```bash
sudo docker image prune -a
```


## Execute API endpoints from inside the container

You need to execute `curl`
```bash
curl http://localhost:80/api/v1/health-check
```


But you can loop through multiple endpoints and hit them one after another with curl.
